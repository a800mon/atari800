...
while allowing headless/remote-oriented behavior during an active remote session.
...
- In remote/headless mode, `Ctrl+C` while paused exits immediately.
- If a remote monitor client disconnects during headless monitor I/O:
  - without pending `SIGINT`: monitor loop exits back to emulation flow,
  - with pending `SIGINT`: emulator terminates.

## Audio During Debug

- `REMOTE_MONITOR_AUDIO_ON_DEBUG=1`: audio may stay active during remote debug live refresh.
