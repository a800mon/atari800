...
- When relevant, explicitly verify compliance with this file:
  ...
  - docs consistency (`DOC/USAGE`, `src/atari800.man`, feature docs, `DOC/NEWS`),
...
- Choose documentation scope by impact:
  ...
  - release summary: add a concise entry to `DOC/NEWS` under `Next version`.
...
- Keep `DOC/NEWS` compact:
  - `New features`: what was added and how to enable it,
  - `Limitations`: platform availability/constraints when relevant,
  - avoid low-level implementation details (internal constants, file paths, transport internals) unless essential for users.
